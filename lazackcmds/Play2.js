let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    let audioUrl;
    try {
      const audioResponse = await fetch(`httpshttps://api.apify.com/v2/acts/easyapi~youtube-video-and-mp3-downloader/runs?token=$AIzaSyD00_9u1ApXlOKYqwBTn_ztMkCOGTw4RUU/search?query=${encodeURIComponent(text)download?url=${encodeURIComponent(video.url)}`)`);
